...
class TwoLayerMLP(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    h = np.exp(scores_mod) / np.sum(np.exp(scores_mod), 1).reshape(-1,1)
    L = scores_mod - np.log(np.sum(np.exp(scores_mod), 1)).reshape(-1,1)
    loss = -np.sum(L[range(N), y]) / N + reg * (np.sum(np.square(W1)) + np.sum(np.square(W2))) / 2
    ###########################################################################
    #                            END OF YOUR CODE
    ###########################################################################
    ...
    yy = np.zeros([N, C])
    yy[range(N), y] = 1
    dscore = h - yy # partial derivative of loss wrt. the logits (dL/dz)
    dW2 = (hidden.T @ dscore) / N + reg * (W2)     # partial derivative of loss wrt. W2
    db2 = dscore.sum(axis=0) / N     # and so on...
  ...
  def predict(self, X):
    ...
    ###########################################################################
    score = np.exp(self.loss(X)) / np.exp(self.loss(X)).sum(axis=1).reshape(-1, 1)
